...
namespace Foundry.Networking
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        public UnityEvent OnPlayerInstantiated;    //Owl

        [HideInInspector] public Player localPlayer;

        [HideInInspector] public GameObject playerObject;                // Owl
        ...
        async void Start()                              // Owl
        {
            if (autoStart)
            {

                await StartSession(networkMode);
                 OnPlayerInstantiatedCallback();
            }
        }

        void OnPlayerInstantiatedCallback()    // Owl
        {
        ...
        public async Task StartSession(SessionType mode)
        {
            ...
            if(mode == SessionType.Shared)
                switch (SpawnMethod)
                {
                    case SpawnMethod.Random:
                        ...
                        {
                            playerObject = this.Instantiate(playerPrefab, spawnPoints[Random.Range(0, spawnPoints.Length - 1)].position,
                        ...
                        else
                        {
                            ...
                            playerObject = this.Instantiate(playerPrefab, transform.position, transform.rotation);
                        }
